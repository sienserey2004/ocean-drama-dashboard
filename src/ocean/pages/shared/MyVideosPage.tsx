...
import { adminVideoApi } from '@/ocean/api/admin.service'
import { categoryApi, tagApi } from '@/ocean/api/categoryTag.service'
import type { Video, Category, Tag, VideoRess } from '@/ocean/types'
  ...
  const openEdit = async (vs: VideoRess) => {
    try {
      ...
      reset({
        ...
        is_free: v.is_free,
        category_ids: v.categories?.map((c: Category) => c.category_id) || [],
        tag_ids: v.tags?.map((t: Tag) => t.tag_id) || []
      })
